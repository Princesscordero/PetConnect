...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $email = $_POST['email'];
    $reason = $_POST['reason'];
    $isNewOwner = isset($_POST['isNewOwner']) ? (int)$_POST['isNewOwner'] : 1;
    $petsOwned = isset($_POST['petsOwned']) ? (int)$_POST['petsOwned'] : 0;

    $stmt = $pdo->prepare("INSERT INTO adoption_requests (pet_name, adopter_name, email, reason, is_new_owner, pets_owned) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->execute([$petName, $yourName, $email, $reason, $isNewOwner, $petsOwned]);
...
<body>
  ...
  <section class="heroa">
    ...
    <p><a href="adoption_requests.php">View All Adoption Requests</a></p>
  </section>
  ...
  <div id="adoptionModal" class="modal" style="display:none;">
    <div class="modal-content">
      ...
      <form method="POST" id="adoptionForm">
        ...
        <label>Are you a new pet owner?</label>
        <div>
          <input type="radio" id="newYes" name="isNewOwner" value="1" required>
          <label for="newYes">Yes</label>

          <input type="radio" id="newNo" name="isNewOwner" value="0">
          <label for="newNo">No</label>
        </div>

        <label for="petsOwned">How many pets do you currently own?</label>
        <input type="number" id="petsOwned" name="petsOwned" min="0" required>
